...
export async function generateStaticParams() {
  return getNodesByCategory("mc-kjop").map((n) => ({ slug: n.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const node = getNode("mc-kjop", slug);
  if (!node) return {};

  const canonical = `${SITE_URL}/mc-kjop/${node.slug}`;
...
export default async function McKjopSlugPage({ params }: Props) {
  const { slug } = await params;
  const node = getNode("mc-kjop", slug);
  if (!node) notFound();
