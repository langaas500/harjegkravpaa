...
export async function generateStaticParams() {
  return getNodesByCategory("flyreiser").map((n) => ({ slug: n.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const node = getNode("flyreiser", slug);
  if (!node) return {};

  const canonical = `${SITE_URL}/flyreiser/${node.slug}`;
...
export default async function FlyreiserSlugPage({ params }: Props) {
  const { slug } = await params;
  const node = getNode("flyreiser", slug);
  if (!node) notFound();
