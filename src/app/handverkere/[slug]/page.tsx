...
export async function generateStaticParams() {
  return getNodesByCategory("handverkere").map((n) => ({ slug: n.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const node = getNode("handverkere", slug);
  if (!node) return {};

  const canonical = `${SITE_URL}/handverkere/${node.slug}`;
...
export default async function HandverkereSlugPage({ params }: Props) {
  const { slug } = await params;
  const node = getNode("handverkere", slug);
  if (!node) notFound();
